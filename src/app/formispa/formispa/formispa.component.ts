...
export class FormispaComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  /** Scale by width only — paginates automatically if content overflows. */
  ...
  async exportToPDF(form: NgForm) {
    ...
    try {
      ...
      const pdfFile  = new File([pdfBlob], `declaracion-impuestos-${uniqueId}.pdf`, { type: 'application/pdf' });
