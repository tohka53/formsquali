...
export class FormbinkComponent implements OnInit {
  ...
  async exportToPDF() {
    try {
      ...
      // 3. Enviar por email
      this.emailService.sendEmailWithPdf(pdfBlob)
        .subscribe({
          next: () => {
            alert('Formulario enviado exitosamente por email');
            // También guardamos localmente el PDF
            pdf.save('formulario-business-intake.pdf');
          },
          error: (error) => {
            console.error('Error:', error);
            alert('Error al enviar el formulario por email');
            // Aún así guardamos el PDF localmente
            pdf.save('formulario-business-intake.pdf');
          }
        });
